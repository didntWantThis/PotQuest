...
void Game::initializeGame(){
	...
	img_data.LoadFromFile ("Images/checker-hurt.jpg");
	...
	stupid = new Model("models/first frame.obj");
	stupid->addKeyFrame("models/second frame.obj");
	this->addToDrawList(player);
	this->addToDrawList(floor);
	this->addToDrawList(stupid);
}
...
void Game::update(){
	...
	camera.eY = player->position.y + 10;
	camera.eZ = player->position.z - 15;
...
void Game::InputHandler(){
	...
	while (App->GetEvent(Event)){
		...
		if (Input.IsKeyDown(sf::Key::Up)){
		 ...
		 if (Event.Type == sf::Event::KeyPressed && Input.IsKeyDown(sf::Key::C)){
			if (stupid->currentMeshIndex == 0){
				stupid->lerp(stupid->KeyframeList[0],stupid->KeyframeList[1],deltaT);
				stupid->currentMeshIndex = 1;
			}
			else{
				stupid->lerp(stupid->KeyframeList[1],stupid->KeyframeList[0],deltaT);
				stupid->currentMeshIndex = 0;
			}
			 
			 /*stupid->previousMeshIndex = stupid->currentMeshIndex;
			if (stupid->currentMeshIndex == 0){
				stupid->currentMeshIndex = 1;
			}
			else{
			...
			stupid->lerp(stupid->KeyframeList[stupid->previousMeshIndex],
				stupid->KeyframeList[stupid->currentMeshIndex], deltaT);*/
		}
	}//while
}
